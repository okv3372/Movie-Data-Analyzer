...
public class MovieMain {
    ...
    /**
     * counter the number of movies per genre
     */
    private final static String GROUP_BY = "GROUP_BY";
    /**
     * count the number of movies with the top number of votes (from ratings)
     */
    ...
    /**
     * Handles the group by command query, e.g. "GROUP_BY MOVIE 1998 2005".
     *
     * @param type  the movie type, e.g. "MOVIE", "TV_SHOW", etc.
     * @param start the start year (inclusive)
     * @param end   the end year (inclusive)
     */
    private void processGroupBy(String type, int start, int end) {
        Map<Integer, Map<Genre, Integer>> movies = this.imdb.getCountOfMoviesByGenre(type, start, end);
        for (int year = start; year <= end; ++year) {
            System.out.println("YEAR: " + year);
            if (movies.containsKey(year)) {
                Map<Genre, Integer> genres = movies.get(year);
                for (Genre genre : genres.keySet()) {
                    System.out.println("\t" +
                            "GENRE: " + genre +
                            ", AMOUNT: " + genres.get(genre));
                }
            }
        }
    }
    ...
    public void mainLoop() {
        ...
        while (in.hasNext()) {
            ...
            switch (fields[0]) {
                ...
                case GROUP_BY -> processGroupBy(fields[1], Integer.parseInt(fields[2]), Integer.parseInt(fields[3]));
