...
public abstract class IMDB {
    ...
    /**
     * For a range of inclusive years, count the number of movies released of a certain type grouped by genre.
     *
     * The collection returned should have stored the number movies of the given type
     * per year and genre.
     *
     * @param type the movie type, e.g. "MOVIE", "TV_SHOW", etc.
     ...
     * for each year is another map keyed by genre ordered alphabetically. The values of the genre maps
     * is the number of movies in that genre.
     */
    public abstract Map<Integer, Map<Genre, Integer>> getCountOfMoviesByGenre(String type, int start, int end);
    ...
    public IMDB(boolean small) throws FileNotFoundException {
        ...
        while (in.hasNext()) {
            ...
            if (!fields[4].equals(IS_ADULT)) {     // ignore adult movies
                ...
                this.movieList.addLast(movie);// append the Movie to the back of this.movieList
            }
        }
        in.close();
    ...
    public void processRatings() throws FileNotFoundException {
        ...
        while (in.hasNext()) {
            String line = in.nextLine();
            String[] fields = line.split("\t");
            Movie movie = findMovieByID(fields[0]);
            if(movie != null){
                movie.setRating(new Rating(fields[0], Double.parseDouble(fields[1]), Integer.parseInt(fields[2])));
            }
        }
        in.close();
